...
void printBPA(BPA e){   printf("BPA variant = %s\n", BPA_str[(std::size_t)e]);}
...
float trainNNetwork_final(NNState *state, float *input, float *desired , int iteration  )
{
	...
	//2. copy back results to cpu to calculate error when needed
	...
	switch (state->variant )
				{
						...
						case RESILIENT :
						{
							for ( i = OUTPUT_LAYER ;    i != SIZE_MAX    ; i--) //
							{
								...
								if (i == OUTPUT_LAYER )
								{
									Kernel_resilient_last<<< 1 , neurons >>>( state->d_Out_ff[i-1] ,  state->d_W[i] , state->d_Out_ff[i] , state->d_desired , state->d_delta_val[i]   , state->d_Grad[i] ,state->d_dij_prev[i] , state->dW_prev[i], iteration ) ;
								}
								else
								{
									Kernel_relilient_hidden<<< 1, neurons >>>(  ( (i==0)? state->d_input_vec : state->d_Out_ff[i-1]  ) , state->d_W[i]
																, state->d_Out_ff[i] , state->d_W[i+1]  , state->d_Out_ff[i+1] , state->d_delta_val[i]  , 	 state->d_delta_val[i+1] , state->d_Grad[i] , state->d_dij_prev[i] ,  state->dW_prev[i] , iteration	) ;
								}
								cudaDeviceSynchronize();
							}
							cudaDeviceSynchronize();
